...
#error unknown compiler!
	...
	if (argc > 0 && srcName != NULL) {
		...
		if ((parser->trace = _wfopen(traceFileName, L"w,ccs=utf-8")) == NULL) {
		...
		// obtain the FileSize
		parser->trace = _wfopen(traceFileName, L"r,ccs=utf-8");
		fseek(parser->trace, 0, SEEK_END);
		long fileSize = ftell(parser->trace);
		fclose(parser->trace);
		if (fileSize == 0) {
			_wremove(traceFileName);
		} else {
